...
const FILTER_DEFAULTS = { tasks: true, requests: true, events: true, completed: false };
...
export default function ParentDashboard() {
  ...
  const taskModalRef = useFocusTrap(showTaskModal);
  ...
  const activeTasks = tasks.filter(t => t.status !== 'completed' && t.status !== 'rejected' && t.status !== 'pending');
    ...
    <div className="page-container">
      <div className="page-header" style={{ marginBottom: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button className="btn btn-primary btn-small" onClick={() => setShowTaskModal(true)}>+ New Request</button>
      ...
      <div className="summary-bar">
        <div className="summary-counts">
          ...
          <span className="summary-item"><strong>{activeTasks.length}</strong> Active Tasks</span>
          <span className="summary-sep" />
      ...
      {allItems.length === 0 ? (
        <div className="empty-state">
          ...
          <p>{!filters.tasks && !filters.events ? 'Select a category above' : 'Send a request or create an event to get started'}</p>
        </div>
      ) : (
        groups.map(group => {
      ...
      {showTaskModal && (
        <div className="modal-overlay" onClick={() => setShowTaskModal(false)}>
          <div ref={taskModalRef} className="modal" role="dialog" aria-modal="true" aria-labelledby="task-modal-title" onClick={e => e.stopPropagation()}>
            <h2 id="task-modal-title">New Request</h2>
...
function TimelineItem({ item, userRole, currentUserId, onUpdateTask, onRespondEvent, onDeleteTask, onDeleteEvent }) {
